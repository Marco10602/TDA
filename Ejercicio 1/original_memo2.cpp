...
void buscarCamino(int i, int j, int n, int m, int sumActual, const vector<vector<int>>& aulas, bool& terminado) {
    ...
    if (i == n - 1 && j == m - 1 && sumActual == 0) {
        ...
        // Mover hacia abajo
        if (i + 1 < n and abs(sumActual+aulas[i + 1][j]) <= pasosRestantes and terminado != true)
            buscarCamino(i + 1, j, n, m, sumActual, aulas, terminado);
        
        if (terminado) return;
        
        // Mover hacia la derecha
        if (j + 1 < m and abs(sumActual+aulas[i][j + 1]) <= pasosRestantes and terminado != true)
...
int main() {
    ...
    for (int k = 0; k < cp; k++) {
        ...
        memo.assign(n+1, vector<unordered_set<int>>(m+1));
