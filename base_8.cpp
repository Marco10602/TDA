...
void buscarCamino(int i, int j, int n, int m, int sumActual, vector<int>& caminoActual, const vector<vector<int>>& aulas, bool& terminado) {
    ...
    int minSuma_posible = sumActual - pasos_restantes;
    int maxSuma_posible = sumActual + pasos_restantes;

    // Poda por rango: si no puedo llegar a 0, corto
    if (0 < minSuma_posible || 0 > maxSuma_posible) {
        caminoActual.pop_back(); // retrocedo
        return;
    }
